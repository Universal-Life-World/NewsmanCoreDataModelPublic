...
extension NMBaseSnippetsTests
{
 ...
 {
  Self.model.create(persist: persisted, objectType: T.self) { result in
   switch result {
 ...
 final func test_snippet_creation_with_geo_locations_services_available()
 {
  ...
  let creationGroup = NMBaseSnippet.SnippetType.allCases.map{ snippetType -> XCTestExpectation in
   ...
   switch snippetType {
    case .base: createHelper(NMBaseSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
     
    case .photo: createHelper(NMPhotoSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
     
    case .audio: createHelper(NMAudioSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
     
    case .video: createHelper(NMVideoSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
     
    case .text: createHelper(NMTextSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
     
    case .mixed: createHelper(NMMixedSnippet.self, persisted, snippetType) { [ unowned self ] in
     suts.append($0)
     expectation.fulfill()
    }
   }
